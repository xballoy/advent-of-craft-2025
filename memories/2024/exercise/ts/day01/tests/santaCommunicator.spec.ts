...
describe('SantaCommunicator', () => {
    ...
    test('composeMessage', () => {
        const message = communicator.composeMessage({...dasher, numbersOfDaysForComingBack: 5}, numberOfDayBeforeChristmas);
    ...
    test('shouldDetectOverdueReindeer', () => {
        const overdue = communicator.isOverdue({...dasher, numbersOfDaysForComingBack: numberOfDayBeforeChristmas}, numberOfDayBeforeChristmas, logger);
    ...
    test('shouldReturnFalseWhenNoOverdue', () => {
        const overdue = communicator.isOverdue({...dasher, numbersOfDaysForComingBack: numberOfDayBeforeChristmas - numberOfDaysToRest - 1}, numberOfDayBeforeChristmas, logger);
